...
The selection is seeded by ``(fov, fov_timestep)`` so the same call always
returns the same patches — useful if the run is replayed offline.

The per-cell patch coordinates (cell label, patch grid index, patch bbox,
dot center) are accumulated in ``self.records`` while the experiment runs.
After acquisition, ``to_dataframe()`` returns them as a DataFrame so the
notebook can merge them into ``exp_data.parquet`` on
``(fov, fov_timestep, label)`` — no separate sidecar file is written.
"""
...
class RandomStimPerCell14pxPatches(StimWithPipeline):
    ...
    def _rng_for(self, metadata):
        ...
        t = (
            int(metadata.get("fov_timestep", metadata.get("timestep", 0)))
            if metadata
            else 0
        )
        seed = (self._base_seed * 2_654_435_761 + fov * 1_000_003 + t * 100_003) & 0xFFFFFFFF
    ...
    def get_stim_mask(self, label_images, metadata=None, img=None, tracks=None):
        ...
        ps = self.PATCH_SIZE
        rng = self._rng_for(metadata or {})

        per_cell = self._select_patch_per_cell(labels, rng)
